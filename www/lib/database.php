...
class BufetData {
	...
	private function __construct() {
		$this->PDO = new PDO('mysql:host=localhost;dbname=m25bufet','bufet','34t411th3c4nd135');
	}
	...
	private function fetchUserData(&$value) {
		...
		if (count($cred) !== 2) return;
		if ($cred[0] !== 'fb') return;
		$encoded_json = file_get_contents("http://graph.facebook.com/$cred[1]?");
		$dec = json_decode($encoded_json);
		if ($value['name'] === '<FETCH>') $value['name'] = $dec->{'name'};
		if ($value['username'] === '<FETCH>') $value['username'] = $dec->{'username'};
		if ($value['picture_url'] === '<FETCH>') $value['picture_url'] = "http://graph.facebook.com/$cred[1]/picture?type=normal";
	...
	public function getItem($iid) {
		$sel = $this->PDO->prepare('SELECT * FROM actual_price WHERE iid = :iid ');
	...
	public function buyItem($uid,$iid,$amount,$ppu) {
		$sel = $this->PDO->prepare('INSERT INTO transactions VALUES (NOW(), :uid, :iid, :amount, :ppu, \'purchase\');');
		$sel->bindValue(':uid',$uid,PDO::PARAM_INT);
		$sel->bindValue(':iid',$iid,PDO::PARAM_INT);
